"""
CODAR Surface Currents Vector Fetcher

This module provides functionality to download CODAR surface current vector data (.tuv files)
from Ocean Networks Canada for a specified time range. It organizes downloads into parallel
batches, tracks progress, and provides error handling and retry mechanisms.

Features:
- Queries ONC for CODAR vector files (.tuv) within a specified time window.
...
Usage:
    python fetch_surface_current_vectors.py --start "2023-01-01T00:00:00.000Z" --end "2023-01-03T00:00:00.000Z"

Command Line Input Options:
- start: Required. Start date in ISO 8601 format (e.g., 2023-01-01T00:00:00.000Z).
- end: Required. End date in ISO 8601 format (e.g., 2023-01-03T00:00:00.000Z).

Output Structure:
    Downloads/
        surface-currents/vectors/
            STRAITOFGEORGIAARRAY_{YYYYMMDDTHHMMSS.sssZ}.tuv
            ...
        surface-currents-metadata/vectors/
            manifest.csv
            provenance.yaml
...
- Manual thread pool with a task queue for efficient parallel downloads.
...
class Config: # TODO: add output dir and extension as variables
...
# NOTE: to get netcdf files for the quality controlled vectors, use:
# VECTOR_PARAMS = {
#     "locationCode": "SOGCS",
#     "deviceCategoryCode": "OCEANOGRAPHICRADAR",
...
class VectorFetcher:
    ...
    def __init__(
        ...
        # Store last API params used for the provenance.
        self.actual_params: dict = {}

    # Manifest / Provenance helpers - Non threading
    ...
    def write_provenance(self) -> None:
        ...
        # Avoid leaking internal client details
        safe_params = {k: v for k, v in self.actual_params.items() if k not in {"token", "method"}}
        provenance = {
    ...
    def get_filenames(self, date_from: str, date_to: str) -> List[str]:
        ...
        self.actual_params = params.copy()
        return filenames
...
def main() -> None:
    ...
    metadata_root = downloads_dir / "surface-currents-metadata/vectors"
    data_root = downloads_dir / "surface-currents/vectors"

    client = onc.ONC(token, outPath=str(data_root))
    fetcher = VectorFetcher(vector_client=client, data_root=data_root, metadata_root=metadata_root)
