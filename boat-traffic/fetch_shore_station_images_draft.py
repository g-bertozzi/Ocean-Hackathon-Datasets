...
ONC Shore Station Still Image Fetcher

This module provides functionality to download still camera images (.jpg) from
Ocean Networks Canada (ONC) for a specified location and time range. It is designed
to efficiently retrieve large image archives by batching requests, downloading
files in parallel, and tracking progress in a persistent manifest.

The script builds a complete list of available image filenames for the requested
period, converts them into structured metadata, and manages downloads using a
multithreaded worker queue. Download state is recorded so the process can be
resumed safely if interrupted.

Features:

* Queries ONC archive endpoints to list available still image files for a location
  and time range, split into approximately six month windows.
* Parses filenames into structured metadata including timestamps, device codes,
  and file paths.
* Downloads images in configurable batches using multiple worker threads.
* Retries failed downloads up to a configurable maximum.
* Periodically checkpoints download progress to disk to prevent data loss.
* Maintains a manifest CSV to track file status (success or failed).
* Writes a provenance YAML file describing API calls, parameters, citations,
  and output artifacts for reproducibility.
...
* Location code, download paths, batch size, and thread count are defined in the
  script and can be adjusted as needed.
* The default run fetches one year of still images at five minute intervals from
  a shore station camera.

Output Structure:
    Downloads/
        boat-traffic/{LOCATION_CODE}/
            DEVICECODE_YYYYMMDDTHHMMSS.jpg
            ...
...
* The manifest file allows the script to resume incomplete or failed downloads
  without restarting from scratch.
* Downloads are written directly to the ONC client output path.
* Ensure the ONC_TOKEN environment variable is set before running.
